...
router.post('/send-all', async (req, res) => {
  try {
    ...
    // Deduplicate by push token: same device may be logged into multiple employee accounts.
    // Without this, Expo receives the same token N times and delivers N notifications to one phone.
    const seenTokens = new Set();
    const messages = [];
    for (const emp of employees) {
      if (!seenTokens.has(emp.pushToken)) {
        seenTokens.add(emp.pushToken);
        messages.push({ to: emp.pushToken, sound: 'default', title, body });
      }
    }
    ...
    try {
      await NotificationLog.create({ title, body, sentTo: 'All Employees', sentBy: 'Admin', recipientCount: messages.length });
    } catch (logErr) {
